...
class TextHoundClient:
	...
	def query(self, query):
		...
		http_conn.request('GET', TEXT_ENDPOINT + '?' + urllib.urlencode({'query': query}), headers = headers)
...
class StreamingHoundClient:
		...
		def start(self, listener):
			...
			self.conn.SendMessage(HTPMessage(HTPMessage.HTP_TYPE_JSON,
					json.dumps({'access_id': self.clientID, 'signature': signature, 'version': '1.0-sh'})))
...
if __name__ == '__main__':
	...
	for fname in sys.argv[3:]:
		...
		samples = audio.read(BUFFER_SIZE)
		finished = False
